  ...
  return (
    <div>
      <nav className="mx-auto my-2 lg:my-4 block md:grid md:grid-cols-3 justify-start backdrop-blur-sm p-2.5 lg:px-8 w-[95%] max-w-[200rem] text-md text-white bg-black/80 font-mono border rounded-md fixed bottom-0 md:top-0 md:bottom-auto left-0 right-0 z-50">
        <h1 className="hidden md:block text-xl">My Store</h1>
        <div className="hidden md:flex gap-4 items-center justify-center text-foreground/50 ">
          <NavLink
            to="/"
        ...
        <div className="grid grid-cols-5 justify-items-center md:flex md:items-center md:justify-end gap-4">
          <input
            type="text"
            className="md:block bg-background border w-[180px] lg:w-full text-sm px-4 py-1 hidden"
            placeholder="search..."
          />
      ...
      <div className="px-4 py-4 md:py-20 lg:px-10 bg-background min-h-screen text-foreground">
        <Outlet />
      </div>
    </div>
  );
